...
public class Pixy {
	...
	public Pixy() {
		...
		pExc = new PixyException("Uh-oh, something broke?");
		values = new PixyPacket();
	}
	public Pixy(I2C argPixy, PixyPacket[] argPixyPacket, PixyException argPixyException, PixyPacket argValues){
		pixy = argPixy;
	...
	public PixyPacket readPacket(int Signature) throws PixyException { //The signature should be which number object in 
		...
		for (int i = 0; i <= 16; i++) {
			...
			if (syncWord == 0xaa55) { //Check is first 2 bytes equal a "sync word", which indicates the start of a packet of valid data
				...
				packets[Sig - 1].SurfaceArea = packets[Sig-1].Width * packets[Sig - 1].Height;
				...
				SmartDashboard.putNumber("SigNumber: ", Signature);
		}
